...
def prepare_run_paths(input_file: Path, candidate_name: str) -> RunPaths:
    ...
    backup_file = (
        BACKUP_DIR / f"{input_file.stem}_backup_{timestamp}{input_file.suffix}"
    )
    output_file = (
        OUTPUT_DIR / f"{safe_candidate}_Teste_Excel_Analista_Compras{input_file.suffix}"
    )
    if output_file.exists():
        output_file = OUTPUT_DIR / (
            f"{safe_candidate}_Teste_Excel_Analista_Compras_"
            f"{timestamp}{input_file.suffix}"
        )
